...
/**
  *
  */
class RefereeScraper(client: Client[IO])(implicit shift: ContextShift[IO])
    extends Http4sClientDsl[IO] {
  val fotballBaseUrl = Uri.uri("https://www.fotball.no")
  def refereeTemplate(fiksId: Int) =
    (fotballBaseUrl / "fotballdata" / "person" / "dommeroppdrag")
      .+?("fiksId", fiksId)
  ...
  def scrapeMatches(fiksId: Int) =
    client.expect[String](refereeTemplate(fiksId)).map { doc =>
      val (refName, urls) =
        RefereeScraper.parseMatches(fiksId, Jsoup.parse(doc))
